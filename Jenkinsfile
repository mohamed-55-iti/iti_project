            ...
            steps {
                withCredentials([usernamePassword(credentialsId: 'github-creds', usernameVariable: 'GIT_USER', passwordVariable: 'GIT_PASS')]) {
                    sh '''
                        ...
                        sed -i "s|image: amrhatem/backend-flask-app:.*|image: ${DOCKER_REGISTRY}/backend-flask-app:${IMAGE_TAG}|g" kubernetes/backend/backend-deployment.yaml
                        sed -i "s|image: amrhatem/frontend-app:.*|image: ${DOCKER_REGISTRY}/frontend-app:${IMAGE_TAG}|g" kubernetes/frontend/frontend-deployment.yaml
